...
public class GarbageCollector {
    public static void main(String[] args) {
        DenisNurmuhametovGraph<Integer> denisNurmuhametovGraph = new DenisNurmuhametovGraph<>();
        ...
        for (int i = 0; i < N; i++) {
            DenisNurmuhametovVertex<Integer> currentVert = new DenisNurmuhametovVertex<>(i);
            directAccessVertices[i] = currentVert;
            denisNurmuhametovGraph.insertVertex(currentVert);
        }

        for (int i = 0; i < N; i++) {
            int pointerFromHere = sc.nextInt();
            if (pointerFromHere >= 0) {
                DenisNurmuhametovVertex from = directAccessVertices[i];
                DenisNurmuhametovVertex to = directAccessVertices[pointerFromHere];
        ...
        for (int i = 0; i < R; i++) {
            DenisNurmuhametovVertex root = directAccessVertices[roots[i]];
            if (!reachable[(Integer) root.label]) {
                denisNurmuhametovGraph.processRoot(root, reachable);
            }
        }

        ArrayList<DenisNurmuhametovGarbagePair> answer = new ArrayList<>();
        int garbageBlocks = 0;
        for (int i = 0; i < N; i++) {
            if (reachable[i] == false) {
                ...
                int garbageLength = 0;
                garbageBlocks++;
                while (i < N && reachable[i] != true) {
                    garbageLength++;
                    i++;
                }
                answer.add(new DenisNurmuhametovGarbagePair(startingIndex, garbageLength));
            }
        }

        System.out.println(garbageBlocks);
        for (int i = 0; i < answer.size(); i++) {
            System.out.println(answer.get(i).startingIndex + " " + answer.get(i).length);
        }

    }
}

class DenisNurmuhametovGarbagePair {
    public int startingIndex;
    public int length;

    public DenisNurmuhametovGarbagePair(int startingIndex, int length) {
...
class DenisNurmuhametovGraph<T> {
    ...
    public void processRoot(DenisNurmuhametovVertex<T> root, boolean reachable[]) {
        denisStack<DenisNurmuhametovVertex<T>> dfsTraversed = new denisStack<>();
        boolean visited[] = new boolean[DenisNurmuhametovVertex.increment];
        DenisNurmuhametov_dfs(root, dfsTraversed, visited, root.adjacentVertices);
        while (!dfsTraversed.isEmpty()) {
            reachable[(Integer) dfsTraversed.pop().label] = true;
        }
    }
...
5) I took my graph implementation from the first exercise and modified it a bit
*/